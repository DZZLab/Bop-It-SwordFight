#include <SPI.h>
#include <Wire.h>
//MPU6050 motion sensor libraries
#include <Adafruit_MPU6050.h>
#include <Adafruit_Sensor.h>
//OLED display libraries
#include <SSD1306Ascii.h>
#include <SSD1306AsciiSpi.h> 
//Serialized LED library
...
void displayCountdownBar(unsigned long elapsedTime, unsigned long totalTime) {
  ...
  int ledLength = (elapsedTime * NUM_LEDS / totalTime);//increases as time goes on
  ...
  for (int j = 0; j <= ledLength; j++){
    strip.setPixelColor(j, strip.Color((255/NUM_LEDS)*j, 0, (255/NUM_LEDS)*j));
    strip.show();
  }
  for (int k = ledLength; k <= NUM_LEDS; k++){
...
void commandSwing() {
  ...
  centerText(command);
  oled.print(command);
  tmrpcm.play("swing.wav"); //audio command
  while (tmrpcm.isPlaying()) {}
  delay(500);
  interrupts();
  unsigned long startMS = millis();
  while (1) {
    ...
    if (mpu.getMotionInterruptStatus()) {
      ...
      tmrpcm.play("coin.wav"); //play points++
      while (tmrpcm.isPlaying()) {}
      noTone(SPEAKER_PIN);
      break;
    }
...
void commandGuard() {
  String command = F("GUARD!");
  noInterrupts();
  oled.clear();
  displayScore();
  centerText(command);
  oled.print(command);
  tmrpcm.play("guard.wav"); //audio command
  while (tmrpcm.isPlaying()) {}
...
void commandCharge() {
  ...
  centerText(command);
  oled.print(command);
  tmrpcm.play("charge.wav"); //audio command
  while (tmrpcm.isPlaying()) {}
  delay(500);//delay to prevent duplicate points
  interrupts();
  unsigned long startMS = millis();
